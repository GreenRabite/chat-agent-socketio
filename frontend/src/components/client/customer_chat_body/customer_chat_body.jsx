...
class CustomerChatBody extends React.Component{
  constructor(props){
    super(props);
  ...
  borderCheck(){
    ...
    for (let i = 0; i < msgs.length; i++) {
      if (i === 0) {
        if (msgs[i][1] === 'client' && msgs[i+1][1] === 'client'){
          chatBorders.push('RD');
        ...
        if (msgs[i][1] === 'client' && msgs[i + 1][1] === 'client' && msgs[i -1][1] === 'client') {
          chatBorders.push('RUD');
        }else if (msgs[i][1] === 'client' && msgs[i + 1][1] === 'client') {
          chatBorders.push('RD');
        }else if (msgs[i][1] === 'client' && msgs[i - 1][1] === 'client') {
          chatBorders.push('RU');
        }else if (msgs[i][1] === 'agent' && msgs[i + 1][1] === 'agent' && msgs[i - 1][1] === 'agent') {
  ...
  render(){
    ...
    if (this.props.messages.length > 1 && this.props.messages.length === this.state.chatBorders.length) {
      const container = document.querySelector(`#client-chat-container-${this.props.roomId}`);
      const chatBubbles = [...container.querySelectorAll('.messages-client')];
